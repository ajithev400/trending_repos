...
const registerUser = async (userData) =>{
    const res = await axios.post(API_URL + 'api/account/', userData)
    return res.data
}

const loginUser = async(userData) =>{
    const res = await axios.post(API_URL + 'api/account/token/', userData)
    if (res.data){
...
const getUser =async ()=> {
  ...
  if(token){
    try{
        const res= await axios.get(API_URL+'api/account/getuser/',{
            headers:{
